...
<div class="animated fadeIn">
  <div class="mt-4">
    <b-card>
      <div slot="header">
        Entry Data Status Pegawai
        <div class="card-header-actions" style="height: 21px;">
          <router-link :to="'/panel/main-data/status-pegawai'" class="btn btn-sm btn-warning text-white"><i class="fa fa-arrow-left" /> Kembali</router-link>
        </div>
      </div>
      ...
      axios.post(this.baseUrl, this.form)
        .then(function (response) {
          ...
          self.$swal({
            ...
            if (result.value) {
              // self.$route
              self.$router.push('/panel/main-data/status-pegawai');
            }
          });
        })
